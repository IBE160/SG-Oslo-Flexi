...
| `latency_sec` | `INTEGER` | Time taken to answer the question. |
...
---
### 5. Adaptive MVP (Lightweight Difficulty Progression)

This outlines a simple, rules-based approach to adapt quiz difficulty without implementing a full Item Response Theory (IRT) model.

- **Difficulty Bands**:
  - Questions are categorized by the Coach Agent into three bands: `EASY`, `MEDIUM`, `HARD`.
  - A new quiz session always starts with a `MEDIUM` question.

- **Progression Rule**:
  - **Bump Up**: If the user answers **3 consecutive questions correctly**, the next question is selected from a higher difficulty band (e.g., `MEDIUM` -> `HARD`).
  - **Bump Down**: If the user answers **2 consecutive questions incorrectly**, the next question is selected from a lower difficulty band (e.g., `MEDIUM` -> `EASY`).
  - The user's difficulty level is maintained for the duration of the quiz session.

- **Logging for Analysis**:
  - The `user_answers` table will log key data for each response:
    - `question_id`: The specific item answered.
    - `difficulty`: The difficulty band of the question.
    - `is_correct`: Boolean result.
    - `latency_sec`: Time in seconds from question view to submission.

- **Post-Session Metrics**:
  - After the quiz, the results screen can display:
    - "You answered 8/10 questions correctly."
    - "You mastered 3 `HARD` questions."
    - "Average time per question: 15 seconds."

---
### 6. Interoperability (Export/Import)
...
---
### 7. Next Steps
- Implement the core quiz-taking flow (Get Quiz -> Submit -> Complete).
