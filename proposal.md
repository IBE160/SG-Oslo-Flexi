...
### Flow 1 - First-Time Student: Upload -> Summarize -> Flashcards -> Quiz -> Feedback
1. Landing -> Get Started: brief value props; click "Get Started".
2. Auth: sign up / login; redirect to "Dashboard".
3. New Session: choose "Upload Document" (PDF/DOCX/TXT <= 15 MB); add title and subject.
4. Reader Agent: extracts sections and key terms; shows "Preview Summary" (bullets and glossary).
5. Generate Learning Pack: creates Summary (1-2 min), Flashcards (10-20), Quiz (5-10 MCQ).
6. Study: Flashcards (Got it / Again, progress bar); Quiz (per-item feedback, final score).
7. Coach Agent Feedback: weak-area tips; "Reinforce Weak Areas" creates targeted cards/quiz.
8. Save and Exit: results stored (score, completion %, time); session appears on Dashboard.

### Flow 2 - Returning Student: Quick Continue
1. Dashboard: "Resume" card shows last sessions and status (e.g., "Quiz 60%").
2. Pick Mode: Flashcards / Quiz / Summary Review; optional "Focus on Weak Topics".
3. Autosave: streak, completion, and trends update; Back to Dashboard.

### Flow 3 - Handwritten Notes via OCR (Photos/Scans)
1. New Session -> Upload Images (JPG/PNG, 1-10); set language (NO/EN) and subject.
2. Vision/OCR: segments headings; flags low-confidence text.
...
4. Generate Learning Pack: Summary, Flashcards, Quiz; transcript kept with session.

### Flow 5 - Quiz-Only Refresh from Existing Material
1. Materials: select any prior session or file.
2. Coach Targeting: toggle "Only concepts I missed" or "Comprehensive".
...
### Global Error and Edge Handling
- Unsupported or large file: inline guidance to export to PDF/TXT; retry button.
- Low OCR confidence: mandatory review step with highlights.
- Generation failure: one auto-retry; fallback "Summary-only" with banner notice.
- Privacy: "Delete data" on session; immediate purge for that user.

### Micro-Flows (Reusable)
- Language Toggle: switch NO/EN output for any pack.
- Export/Share: copy summary; export flashcards to CSV/Anki.
- Session Merge: combine two sources into one learning pack (experimental).
